...
import {
  ...
  fast-data-engineSettings,
  GlobalValidators,
  setupForMaterialUI,
  usefast-data-engine
} from '../fast-data-engine';
...
describe('Form Validation', () => {
  ...
  it(`Error raised when input has invalid value and Enter is pressed`, () => {
    const FormWrapper = () => {
      formInfo = usefast-data-engine(submitHandler);
  ...
  it('Validators can be negated', () => {
    const FormWrapper = () => {
      formInfo = usefast-data-engine(submitHandler, { field1: '', field2: '' }, null);
  ...
  [
    ...
    it(`Can handle ${t.name} initial form value.`, () => {
      const FormWrapper = () => {
        formInfo = usefast-data-engine(submitHandler, t.value, null);
  ...
  [
    ...
    it(`Can handle ${t.name} form submission handler.`, () => {
      const FormWrapper = () => {
        formInfo = usefast-data-engine(t.value, {}, null);
  ...
  it('Can handle form with no inputs: form is always valid', () => {
    const FormWrapper = () => {
      formInfo = usefast-data-engine(submitHandler, {}, null);
  ...
  it('Global custom error attribute is added to input', () => {
    const originalInvalidAttr = fast-data-engineSettings.invalidAttr;
    fast-data-engineSettings.invalidAttr = { 'input-has-error': 'yes' };
    ...
    fast-data-engineSettings.invalidAttr = originalInvalidAttr;
  });

  it('Form submission is prevented when there were errors in the form', () => {
    const FormWrapper = () => {
      formInfo = usefast-data-engine(null, { field1: '', field2: '' });
  ...
  it('Custom validator function mixed with built in validator by key can be provided together', () => {
    const FormWrapper = () => {
      formInfo = usefast-data-engine(submitHandler, { field1: '', field2: '' });
      return buildTestForm(
        formInfo,
        [
          'isRequired',
  ...
  it(`Correct error is thrown when user-provided submit handler throws an exception`, () => {
    ...
    const FormWrapper = () => {
      formInfo = usefast-data-engine(submitHandlerThatThrows, {
        field1: 'test',
        field2: ''
      });
      return buildTestForm(formInfo, ['isRequired'], []);
  ...
  it('Can specify built-in validator without a form submit handler', () => {
    ...
    const FormWrapper = () => {
      formInfo = usefast-data-engine(null, {
  ...
  it('Handle unknown validator properly', () => {
    ...
    expect(callMount).toThrowError(
      new Error(
        `fast-data-engine: cannot find a validator named "unknownValidator". If you are attempting to perform a validation defined by the Validator library, please make sure to have it installed prior.`
      )
    );
  });
  ...
  it(`The setupForMaterialUI() function configures settings correctly`, () => {
    const originalInvalidAttr = fast-data-engineSettings.invalidAttr;
    const originalHelperTextAttr = fast-data-engineSettings.helperTextAttr;
    setupForMaterialUI();

    // still valid because we didn't fin the validation
    expect(fast-data-engineSettings.invalidAttr).not.toBeNull();
    expect(fast-data-engineSettings.invalidAttr.error).toBe(true);

    expect(fast-data-engineSettings.helperTextAttr).not.toBeNull();
    expect(fast-data-engineSettings.helperTextAttr).toBe('helperText');

    fast-data-engineSettings.invalidAttr = originalInvalidAttr;
    fast-data-engineSettings.helperTextAttr = originalHelperTextAttr;
  });
  ...
  [
    ...
    it(`Handle missing optional validator library dependency correctly when using ${v.name}`, () => {
      ...
      try {
        expect(callMount).toThrowError(
          new Error(
            `fast-data-engine: cannot find a validator named "isEmail". If you are attempting to perform a validation defined by the Validator library, please make sure to have it installed prior.`
          )
        );
      } finally {
        jest.resetModules();
  ...
  it('Handle unsupported validator library dependency correctly', () => {
    ...
    try {
      expect(callMount).toThrowError(
        new Error(
          `fast-data-engine: unsupported version of the validator library found (1.0.0). Please upgrade to 4.0.0 or higher.`
        )
      );
    } finally {
      jest.resetModules();
